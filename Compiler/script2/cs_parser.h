...
private:
    ...
    public:
        ...
        inline bool AllOpsPending() const { return ScT::kNone != _scType; };
    ...
    void SetCompileTimeLiteral(Symbol lit, ExpressionResult &xres);
    ...
    void HandleStructOrArrayResult(ExpressionResult &xres);

    // If the result isn't in AX, move it there. Dereferences a pointer
    void ExpressionResultToAx(ExpressionResult &xres);
    ...
    bool ParseExpression_CompileTime(Symbol op_sym, ExpressionResult const &xres_lhs, ExpressionResult const &xres_rhs, ExpressionResult &xres);
    ...
    void ParseExpression_New(SrcList &expression, ExpressionResult &xres);

    // Parse the term given in 'expression'. The lowest-binding operator is unary '-'
    // 'expression' is parsed from the beginning. The term must use up 'expression' completely.
    void ParseExpression_PrefixMinus(SrcList &expression, ExpressionResult &xres);

    // Parse the term given in 'expression'. The lowest-binding operator is unary '+'
    // 'expression' is parsed from the beginning. The term must use up 'expression' completely.
    void ParseExpression_PrefixPlus(SrcList &expression, ExpressionResult &xres);

    // Parse the term given in 'expression'. The lowest-binding operator is a boolean or bitwise negation
    // 'expression' is parsed from the beginning. The term must use up 'expression' completely.
    void ParseExpression_PrefixNegate(Symbol operation, SrcList &expression, ExpressionResult &xres);

    // Parse the term given in 'expression'. The lowest-binding operator is '++' or '--'.
    // 'expression' is parsed from the beginning. The term must use up 'expression' completely.
    void ParseExpression_PrefixCrement(Symbol op_sym, SrcList &expression, ExpressionResult &xres);
    // Parse the term given in 'expression'. The lowest-binding operator is '++' or '--'.
    // 'expression' is parsed from the beginning. The term must use up 'expression' completely.
    void ParseExpression_PostfixCrement(Symbol op_sym, SrcList &expression, ExpressionResult &xres);
    ...
    // 'expression' is parsed from the beginning. The term must use up 'expression' completely.
    void ParseExpression_Prefix(SrcList &expression, ExpressionResult &xres);
    ...
    void ParseExpression_Postfix(SrcList &expression, ExpressionResult &xres, bool result_used);

    void ParseExpression_Ternary_Term2(ExpressionResult &xres_term1, bool term1_has_been_ripped_out,
        SrcList &term2, ExpressionResult &xres_term2, bool result_used);
    ...
    // 'expression' is parsed from the beginning. The term must use up 'expression' completely.
    void ParseExpression_Binary(size_t op_idx, SrcList &expression, ExpressionResult &xres);
    ...
    // 'src' may be longer than the expression. In this case, leave src pointing to last token in expression.
    // 'src'  is parsed from the point where the cursor is.
    void ParseIntegerExpression(SrcList &src, ExpressionResult &xres, std::string const &msg = "");
    ...
    void AccessData_ProcessArrayIndexIfThere(SrcList &expression, ExpressionResult &xres);

    void AccessData_Variable(VariableAccess access_type, SrcList &expression, ExpressionResult &xres);
    ...
    void AccessData_FirstClause(VariableAccess access_type, SrcList &expression, ExpressionResult &xres, bool &implied_this_dot);
    ...
    void AccessData_SubsequentClause(VariableAccess access_type, bool access_via_this, SrcList &expression, ExpressionResult &xres);
    ...
    void AccessData(VariableAccess access_type, SrcList &expression, ExpressionResult &xres);
    ...
    void AccessData_AssignTo(SrcList &expression, ExpressionResult xres);
